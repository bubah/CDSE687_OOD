///////////////////////////////////////////////////////////////////////////////
//        File: map_process_unit_test.cpp
// Description: Test all classes/funcions in the map process executable
//    Language: Visual C++
...
TEST(TestEmit, Test_helper_func_Emit) {
	...
	for (int index = 0; index < words.size(); index++) {
		Emit(words[index], value[index], dm_obj, 3, 
			buffered_memory, num_partitions);
	}
	...
	for (int index = 0; index < words.size(); index++) {
		Emit(words[index], value[index], dm_obj, 4, buffered_memory,
			num_partitions);
	}
...
struct Test_Client : public testing::Test{
	//Client *cl_obj = new Client("127.0.0.1", 54);
